...
- useContext()，组件之间共享状态。
- useReducer()，React 本身不提供状态管理功能，通常需要使用**外部库**。这方面最常用的库是 Redux。
- useEffect()，useEffect()：副作用钩子，替代 componentDidMount，每次组件渲染时，就会执行 useEffect()。
...
1. 函数式编程思维方式，赋予 stateLess 组件以状态管理；
2. stateLess 组件更轻量；
