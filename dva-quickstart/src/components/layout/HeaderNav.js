...
class Header extends React.Component {
  ...
  componentWillReceiveProps(props) {
    if (!this.state.menus.length) {
      ...
      if (props.activeModule) {
        ...
        this.setState({
          menus: props.menus || [],
          activeModule: props.menus && props.menus.length > 0 ? props.menus[0].parentName : '',
        })
      }
    }
  }
  ...
  getAppList(){
    var apps =  [{appName: "轻松有钱app", appCode: "1"},{appName: "金壹贷app", appCode: "0"}];
    this.setState({
      apps:apps,
      appName:sessionStorage.getItem('appName') == null?apps[0].appName:sessionStorage.getItem("appName"),
      appCode:sessionStorage.getItem('appCode') == null?apps[0].appCode:sessionStorage.getItem("appCode")
    },()=>{
    ...
    fetchPost("/admin/productCategory/list").then((res) => {
      ...
      if (res.code === 0) {
        ...
        if (data.length > 0) {
          this.setState({
            categories: data,
            productCategory: sessionStorage.getItem('productCategory') == null ? data[0].productCategory : sessionStorage.getItem('productCategory'),
            productCategoryName: sessionStorage.getItem('productCategoryName') == null ? data[0].productCategoryName : sessionStorage.getItem('productCategoryName'),
          }, () => {
  ...
  getCategorySwitch() {
    ...
    this.state.categories.map((category) => {
      data.push(<Menu.Item style={{paddingLeft: 15}} key={category.productCategory}>{category.productCategoryName}</Menu.Item>);
    });
    return data;
  }
  ...
  render() {
    ...
    //     </Menu>
    // );
    const menu = (<Menu onClick={(e) => this.handleAppClick(e)}>
      {this.getCategorySwitch()}
    </Menu>);
    ...
    return (
      <div className={styles.headerWrapper}>
        ...
        >
          ...
          <h3 style={{textAlign: 'center', fontSize: '20px'}}>{ this.state.activeModule }</h3>
        </div>
                ...
                */}
                  <Dropdown overlay={menu}>
                    <Button
                      ...
                      style={{
                        paddingLeft: '10px',
                        paddingRight: '10px', height: '38px', color: "#fff"}}
                    >
