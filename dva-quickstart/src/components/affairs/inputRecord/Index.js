...
class Index extends React.Component {
  ...
  getData() {
    ...
    fetchPost(`/settle/fund/listManualRecord?currentPage=${currentPage}&pageSize=${pageSize}`, params).then((res) => {
      if(res.code === 0) {
        // 数据请求成功
        if (res.data !== null) {
  ...
  sendParamsUpdate(params) {
    const { btnType, id, repaymentDate, capital, interest, managerFee, extraInfo } = params;
    ...
    return (
      <div className={Styles.content}>
        <div className={Styles.top}>
          <Search searchParams={this.state.searchParams} changeParams={(params)=>this.changeParams(params)} showAllReview={this.state.showAllReview} searchFunc={(params) => this.searchFunc(params)}/>
        </div>

        <Table bordered pagination={pagination} columns={columns1} dataSource={inputRecordData} />
