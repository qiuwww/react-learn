...
class Paid extends React.Component {
  constructor (props) {
    ...
    this.state = {
      ...
      params: {
        paidStatus: '',
        applyStartDate: moment().subtract('days', 7).format('YYYY-MM-DD'),
        findtype: '',
        inputValue: '',
        applyEndDate: moment().add('days', 1).format('YYYY-MM-DD')
      },
      page: {
        currentPage: 1,
        pageSize: 20,
  ...
  getData () {
    ...
    function GetList() {
      return new Promise((resolve, reject) => {
        fetchPost(`/finance/paying/list?currentPage=${self.state.page.currentPage}&pageSize=${self.state.page.pageSize}`,copyState).then((res) => {
          if(res.code === 0) {
            resolve(res)
          } else {
    ...
    function GetPanel() {
      return new Promise((resolve, reject) => {
        fetchPost('/finance/pay/stat',{paidStatus: self.state.params.paidStatus}).then((res) => {
          if(res.code === 0) {
            resolve(res)
          } else {
            message.error(res.msg)
      ...
      },
      {
        ...
        render: (text,record, index) => {
          if(this.state.params.paidStatus !== '-1') {
            return (
              <div>
                <Button disabled={record.disabled} type="primary" onClick={()=>{this.confirmPay(record, 'lianlian', index)}}>连连打款</Button>
                <Button disabled={record.disabled} className='ml5' type="default" onClick={()=>{this.confirmPay(record, 'koudai', index)}}>口袋打款</Button>
              </div>
            )
          }
        }
