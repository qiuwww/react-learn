...
class RepaymentList extends React.Component {
  ...
  getTableData() {
    ...
    return new Promise((resolve, reject) => {
      fetchPost(`/order/repayment/page?currentPage=${pagination.currentPage}&pageSize=${pagination.pageSize}`, {
        ...self.state.params
      }, 'POST').then((res) => {
        if (res.code === 0) {
          self.setState({
      ...
      <Card bordered={false} className={Styles.wrap}>
        <Form className={Styles.header}>
          <Row gutter={16}>
            ...
            <Col span="3" >
              <FormItem >
                <Button type="primary" onClick={this.searchHandler.bind(this)}>搜索</Button>
              </FormItem>
            </Col>
