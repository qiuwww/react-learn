...
import {Table, Button, message, Form, Input, DatePicker, Select, Modal, Upload, Icon} from 'antd';
...
class LineCapitalList extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      selectfundCode: '',
      selectStartTime: Moment().subtract(7, "days").format("YYYY-MM-DD 00:00:00"),
      endStartTime: Moment().format("YYYY-MM-DD 23:59:59"),
      verifiedStartTime: Moment().subtract(7, "days").format("YYYY-MM-DD 00:00:00"),//审核开始时间
    ...
    const columns = [
      {
        ...
        render: (text, record) => {
          return (
            <a className='operateBtn' onClick={() => {
              this.showModal(record)
            }}
               href='javascript:;'>详细</a>
          )
        }
      }]
    const columns02 = [
    ...
    return (
      <div>
        ...
        <Form layout="inline" style={{padding: '10px 20px'}}>
          <FormItem>
            <Upload {...excelPrams}>
              <Button><Icon type="upload"/>导入订单文件(excel)</Button>
            </Upload>
          </FormItem>
          <FormItem>
