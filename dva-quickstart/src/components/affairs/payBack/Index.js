...
import {Table, Button, Modal, Form, message, Col, Input, DatePicker, Select} from 'antd';
...
class PayBack extends React.Component {
  ...
  getPaybackItem(record, time) {
    ...
    if (record) {
      ...
      url = `/liquidation/${this.state.currentTradeNo}/repayment/info?paymentDt=${moment().format('YYYY-MM-DD HH:MM:SS')}`
    } else {
      url = `/liquidation/${this.state.currentTradeNo}/repayment/info?paymentDt=${time}`
    }
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => (
          <div>
            {`${record.realCapital}/${record.duration}`}
          </div>
        )
      },
      {
            ...
            return record.repaymentPlan.map((item) => {
              return (
                <div key={item.repaymentDate}>
                  <span>{item.repaymentDate} / {item.actualRepaymentDate}</span>
                </div>
              )
            })
          }
    ...
    return (
      <div>
        <Modal visible={this.state.visible} okText='确认还款' onOk={() => {
          this.handleOk()
        }} onCancel={() => {
          this.handleCancel()
        }}>
          <Form className={Styles.formWrapper}>
            <div className={Styles.operationHeader}>
              ...
              <span className="ml20">订单号: {this.state.repaymentInfo.orderNo}</span>
            </div>
            ...
            <div className={Styles.info}>
              <div className={Styles.wrapper}>
                ...
                <Col span={12} className='mt5'>
                  借款期限:{this.state.repaymentInfo.startDt} ~ {this.state.repaymentInfo.repaymentDt}
                </Col>
              ...
              <FormItem className='mt20' labelCol={{span: 4}}
