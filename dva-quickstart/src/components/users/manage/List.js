    ...
    return (
      <div>
        <a href="JavaScript:void(0);">
          {count} / <span style={{ color }}>{overCount}</span>
        </a>
      </div>
    );
  }
  ...
  render() {
        ...
        render: (value, record) => {
          ...
          return value ? (
            <Popconfirm
              ...
              title={`确认${text}吗？`}
              onConfirm={() =>
                this.handleChangeBlack("delete", record.userCode)
              }
            >
              <a>{text}</a>
            </Popconfirm>
          ) : (
            <a onClick={() => this.handleShowBlackModal(record.userCode)}>
              {text}
            </a>
          );
        }
    ...
    return (
      <div className="ant-layout-container">
        <Card
          bodyStyle={{ padding: 0, height: 1300, width: "auto" }}
          bordered={false}
          noHovering={true}
        >
          <Table
            ...
            >
              {getFieldDecorator("userClass", {
                ...
                <Select style={{ width: 200 }}>
                  {blackSelect.map(item => (
                    <Select.Option key={item.type}>{item.desc}</Select.Option>
                  ))}
                </Select>
              )}
            </FormItem>
