          ...
          switch (type) {
            case 'pass':
              if (this.state.processNode === '1') {
                fetchPost('/audit/operate/skippass', {...operationParams, targetNode: 3}).then((json) => {
                  if (json.code === 0) {
                    self.clearReason()
                    this.state.tagIDs = {}
            ...
            case 'directPass':
              fetchPost('/audit/operate/skippass', {...operationParams, targetNode: 4}).then((json) => {
                if (json.code === 0) {
                  self.clearReason()
                  this.state.tagIDs = {}
                  self.changeOrder('pass', self.state.id)
    ...
    return (
      <span>
        ...
        </span>
        {
          ...
          this.state.stepType !== 'refuseOrder' && this.state.userId ? <Button type='primary'
        ...
        >
          <Form>
            ...
            <FormItem className={Styles.operationHeader}>
              ...
              {
                this.state.processNode !== '2' && this.state.processNode !== '3'
                  ? <Popconfirm placement='top' title='确认通过到照片审核?'
                                onConfirm={() => {
                                  this.operation('pass')
                                }}>
                  <Button type='primary' className='ml20'>照片审核</Button>
                </Popconfirm>
                  : ''
              }
              ...
              {
                this.state.processNode !== '2' && this.state.processNode !== '3'
                  ? <Popconfirm placement='top' title='确认等待电审?'
                                onConfirm={() => {
                                  this.operation('wait')
                                }}>
                  <Button type='default' className='ml10'>等待电审</Button>
                </Popconfirm>
                  : ''
              }
              ...
              <Popconfirm placement='top' title='确认直接全部通过?'
                          onConfirm={() => {
                            this.operation('directPass')
                          }}>
                <Button type='primary' className='ml10'>直接全部通过</Button>
              </Popconfirm>
            </FormItem>

            <FormItem>
              {getFieldDecorator('checkReason',
                {rules: [{required:this.state.currentType=='refuse'?true:false, message: '请输入原因',}]})(
                ...
                <TabPane tab='个人信息 / 通讯录' key='detail'>
                  {
                    this.state.activeKey === 'detail'
                      ...
                      {<Col span={14}>
                        <Card title='' bodyStyle={{padding: '5px'}} noHovering={true}>
                      ...
                      <Col span={10}>
                        <Card title='通讯录' bodyStyle={{padding: '5px'}} noHovering={true}>
                          <Relation id={this.state.id}
                                    userId={this.state.userId}/>
                          <Contacts id={this.state.id}
                                    userId={this.state.userId}/>
                        </Card>
                      </Col>
                    </div>
                ...
                <TabPane tab='运营商 / 风控 / 照片/数据' key='carrier'>
                  {
                    this.state.activeKey === 'carrier'
                      ...
                      <Col span={12}>
                        <Risk item={this.state.item} userId={this.state.userId}
                              id={this.state.id}/>
