...
class Index extends React.Component {
  ...
  getData () {
    fetchPost(`/admin/role/list?currentPage=${this.state.page.currentPage}&pageSize=${this.state.page.pageSize}`, this.state.params).then(json => {
        if (json.data != null) {
  ...
  render () {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => {
          return (
            <a className='operateBtn' onClick={() => { this.editRole(record) }}
    ...
    return (
      <div>
        <div className={Styles.title}>
          ...
          角色列表
          <Button className='ml20' onClick={() => { this.editRole() }}
            type='primary'>
            新增角色
          </Button>
        </div>
        <div className={Styles.searchWrapper}>
          <Search changeItem={(value) => {
