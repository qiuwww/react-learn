...
class MessageSearch extends Component {
  ...
  addBusiness() {
    fetchPost("/business/addBusinessTemplateInfo", this.state.record).then(
      json => {
        if (json.code != 200) {
  ...
  getServicerList = appId => {
    ...
    fetchPost("/servicer/getListByServicer", param ? param : null).then(
      json => {
        if (json.data != null) {
          this.setState({
            servicerList: json.data || []
  ...
  handleVisOk = e => {
    this.props.form.validateFields((err, values) => {
      if (!err) {
        // 判断是新增还是修改
        if (this.state.newModalVis) this.addBusiness();
        this.setState({
          newModalVis: false
        });
      }
  ...
  templateChange = (value, option) => {
    for (var i = 0, len = this.state.templateList.length; i < len; i++) {
      if (this.state.templateList[i].msgTmpId === value) {
        this.state.record.bizFunctionId = parseInt(
          this.state.templateList[i].bizId
        );
      }
    }
    // templateList
        ...
        return (
          <TreeNode title={item.title} key={item.key} dataRef={item}>
            {this.renderTreeNodes(
              item.children ? item.children : item.appInfoVoList
            )}
          </TreeNode>
        );
      }
  ...
  render() {
    ...
    const templateOptions = this.state.templateList.map(d => (
      <Select.Option value={d.msgTmpId}>
        {d.bizName}:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{d.content}
      </Select.Option>
    ));
    const ruleOptions = this.state.ruleList.map(d => (
      <Select.Option value={d.ruleId}>{d.sendRuleName}</Select.Option>
    ));
    const servicerOptions = this.state.servicerList
      ? this.state.servicerList.map(d => (
          <Select.Option value={d.asRefId}>{d.channelName}</Select.Option>
        ))
      : null;
      ...
      <Card>
        <Row>
          ...
          <Col span={20}>
            <Table
              ref="table"
              dataSource={this.state.detailDate}
              style={{ position: "relative" }}
              columns={columns}
              bordered
              rowKey="businessRefId"
              pagination={pagination}
              footer={() => (
                <div>
                  <Button onClick={this.showNewModal.bind(this)}>
                    新增配置
                  </Button>
                  <Popconfirm
                    title="确定重新加载配置数据?"
                    onConfirm={() => this.onReloadCache()}
                    onCancel={this.enterIconCanceling}
                  >
                    <Button
                      type="danger"
          ...
          <Form>
            ...
            <FormItem {...formItemLayout} label="服务商通道">
              {getFieldDecorator("appServicerId", {
                initialValue: this.state.record
                  ? this.state.record.appServicerId || ""
                  : "",
                rules: [{ required: true, message: "请选择服务商通道!" }]
              })(
