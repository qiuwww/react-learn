...
@connect(({ channel, overdue }) => ({
  selectList: channel.selectList,
  list: overdue.list,
  isNewUserList: overdue.isNewUserList,
  pagination: overdue.pagination,
...
export default class Overdue extends Component {
  ...
  render() {
    ...
    const { selectList, list, pagination, loading, isNewUserList } = this.props;
    const columns = [
      {
        title: '应还日期',
        dataIndex: 'dataTime',
      ...
      // }, {
        ...
        title: '详情',
        key: 'details',
        render: (record) => record.dataTime === '全部' ? '' : <Link to={`/operation/overdue/statistical-details?dataTime=${record.dataTime}`}>查看</Link>,
      }
    ];
    return (
      <Card bordered={false}>
        <Row style={{ marginBottom: 20 }}>
          ...
          <Col sm={6} xs={24} style={{ marginBottom: 20 }}>
            ...
            >
              {isNewUserList.map(item => <Option key={item.type.toString()} value={item.type.toString()}>{item.desc}</Option>)}
            </Select>
          </Col>
        </Row>
        <Table
