...
import {Popover, Card, Table, Icon, Collapse} from 'antd'
...
class Photo extends React.Component {
        ...
        <div key={Math.random().toString(16).substring(2)} style={{width: 200, display: 'inline-block'}}
          ...
          <Card style={{width: 200}} bodyStyle={{padding: 0}} noHovering={true}>
            <div className={styles.customImage}>
              <img alt='example' height='150' width='100%'
                   src={item.key}/>
            </div>
  ...
  info() {
    const content = (
      <h3>
        <strong>Esc：退出全屏&nbsp;&nbsp;← →:切换图片 &nbsp;&nbsp;↑ ↓:放大缩小图片  &nbsp;&nbsp;Ctrl+1:重置图片  &nbsp;&nbsp;Ctrl+←
          →:左右旋转</strong>
      </h3>
    )
          ...
          <Collapse defaultActiveKey={['1', '2', '3']}>
            <Panel header={this.info()} key='1'>
              <div style={{maxHeight: 195, overflow: 'auto'}}>
                {this.createPhoto(this.state.record.holdCardimages, images, slf)}
              </div>
