...
class OverdueOperateRecord extends Component {
  ...
  componentWillReceiveProps (props) {
    if (this.state.id != props.id || props.refreshOverdueRecord) {
      this.setState({
        id: props.id
      }, () => {
        this.getData()
  ...
  getData () {
    ...
    if (this.state.id) {
      fetchPost(`/collection/operate/record/${this.state.id}/query`).then(json => {
        if (json.code === 0) {
          if (json.data != null) {
            this.setState({
  ...
  render () {
    ...
    return (
      <div className={Styles.overdueOperateRecord}>
        <Card title='催收操作记录' extra={this.message()} bodyStyle={{padding: 1}} noHovering={true}>
          <Table
            pagination={false}
            bordered
            size='small'
...
export default OverdueOperateRecord
